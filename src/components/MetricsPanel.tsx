...
export const MetricsPanel = ({ data, position }: MetricsPanelProps) => {
  ...
  if (position === 'left') {
    ...
    return (
      <div className="flex flex-col gap-1.5">
        <div className="bg-white/95 border border-slate-200 rounded-lg shadow-sm p-2.5">
          ...
          <div className="space-y-2">
            <div className="bg-amber-50 rounded-lg p-2.5 border border-amber-200">
              <div className="text-xs text-amber-800 font-medium mb-1">TOTAL ASSET</div>
            ...
            <div className="border-t border-slate-200 pt-2">
              <div className="text-xs text-slate-700 mb-1.5 font-medium">POSITION</div>
              {data.holding.isHolding ? (
                <div className="space-y-1 bg-blue-50 rounded-lg p-2 border border-blue-200">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-bold text-slate-800">{formatCurrency(data.holding.buyPrice!)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                ...
                <div className="px-2.5 py-1.5 bg-slate-100 text-slate-600 rounded text-xs font-bold inline-block border border-slate-200">
                  NO POSITION
                </div>
              )}
            </div>
        ...
        <div className="bg-white/95 border border-emerald-200 rounded-lg shadow-sm p-2.5">
          ...
          {strategy ? (
            <div className="space-y-1.5">
              ...
              <div className="space-y-1">
                ...
                <div className="pt-1">
                  <div className="text-xs font-bold text-slate-600 mb-1">EMA Above</div>
                    ...
                    {MULTI_TF_CONDITIONS.map(({ key, label }) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-[10px] font-medium text-slate-700 mt-0.5">{label}</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                <div className="pt-1">
                  <div className="text-xs font-bold text-slate-600 mb-1">Slope Up</div>
                  <div className="grid grid-cols-2 gap-1">
                    {SLOPE_CONDITIONS.map(({ key, label }) => {
                      ...
                      return (
                        <div
                          key={key}
                          className={`flex items-center justify-center gap-1.5 px-2 py-1.5 rounded border ${
                            met
                              ? 'bg-emerald-50 border-emerald-300'
                              : 'bg-slate-50 border-slate-200'
                          }`}
                ...
                <div className="pt-1">
                  <div className="text-xs font-bold text-slate-600 mb-1">BBW</div>
                  <div className="grid grid-cols-2 gap-1">
            ...
            <div className="flex items-center justify-center h-12 text-slate-500 text-sm">
              Waiting...
            </div>
          )}
        </div>

        <div className="bg-white/95 border border-blue-200 rounded-lg shadow-sm p-2.5">
          <div className="flex items-center justify-between mb-1.5">
          ...
          {strategy?.sellConditions ? (
            <div className="space-y-1.5">
              <div className={`flex items-center justify-between px-2 py-1.5 rounded border ${
                ...
                <span className="text-xs font-medium text-slate-700">Dead Cross</span>
                {strategy.sellConditions.dead_cross.met ? (
              ...
              <div className="pt-1">
                <div className="flex items-center justify-between mb-1">
                  <div className="text-xs font-bold text-slate-600">Early Exit</div>
                  <span className={`text-xs font-bold px-2 py-1 rounded ${
  ...
  if (position === 'trades') {
    ...
    return (
      <div className="bg-white/95 border border-amber-200 rounded-lg shadow-sm p-2.5">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-sm font-bold text-slate-800">Recent Trades</h3>
        ...
        <div className="space-y-1 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-400 scrollbar-track-transparent" style={{ maxHeight: '160px' }}>
                  ...
                  <div className="bg-blue-50 border border-blue-300 rounded p-1.5">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex flex-col items-end">
                        <span className="text-xs font-bold text-slate-800">{formatCurrency(trade.price)}</span>
                        <span className="text-[10px] text-slate-500">{formatLocalDateTime(trade.timestamp)}</span>
                      </div>
                    </div>
                  </div>
                ) : (
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="bg-white/95 border border-amber-200 rounded-lg shadow-sm p-2.5">
        ...
        <div className="space-y-2">
          {data.metrics.portfolioReturnWithCommission !== undefined && (
            <div className="bg-gradient-to-br from-emerald-100 to-teal-100 p-2.5 rounded-lg border border-emerald-300">
              <div className="text-xs text-emerald-800 font-bold mb-1">NET PROFIT</div>
              <div
          ...
          <div className="space-y-1">
            <div className="flex justify-between items-center bg-amber-50 p-1.5 rounded border border-amber-200">
              <span className="text-xs text-slate-700 font-medium">Portfolio Return</span>
              <span className={`text-sm font-bold ${
            ...
            <div className="flex justify-between items-center bg-amber-50 p-1.5 rounded border border-amber-200">
              <span className="text-xs text-slate-700 font-medium">Market Change</span>
              <span className={`text-sm font-bold ${
            ...
            <div className="flex justify-between items-center bg-amber-50 p-1.5 rounded border border-amber-200">
              <span className="text-xs text-slate-700 font-medium">Avg Trade Return</span>
              <span className={`text-sm font-bold ${
      ...
      <div className="bg-white/95 border border-blue-200 rounded-lg shadow-sm p-2.5">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-sm font-bold text-slate-800">Statistics</h3>
          <Target className="w-4 h-4 text-blue-600" />
        </div>

        <div className="space-y-1.5">
          <div className="flex justify-between items-center bg-emerald-50 p-2 rounded border border-emerald-300">
            <span className="text-xs text-emerald-700 font-bold">Profit (TP)</span>
            <span className="text-base font-bold text-emerald-600">{data.metrics.takeProfitCount}</span>
          </div>
          <div className="flex justify-between items-center bg-rose-50 p-2 rounded border border-rose-300">
            <span className="text-xs text-rose-600 font-bold">Loss (SL)</span>
          ...
          <div className="border-t border-blue-200 pt-1.5 mt-1.5">
            <div className="text-xs text-slate-700 mb-1.5 font-bold">WIN RATE</div>
            <div className="flex items-center gap-2">
              <div className="flex-1 bg-slate-200 rounded-full h-3 overflow-hidden">
                <div
                  className="bg-gradient-to-r from-amber-500 to-orange-500 h-3 transition-all duration-500"
                  style={{ width: `${winRate}%` }}
                />
              </div>
