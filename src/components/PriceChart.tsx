...
export const PriceChart = ({ data: rawData, onTradeHover, onTimeframeChange, darkMode = false, v10Strategy }: PriceChartProps) => {
  ...
  const chartContent = (
    <div className={`${colors.chartBg} rounded-xl shadow-lg border ${colors.chartBorder} w-full overflow-hidden ${isMaximized ? 'fixed inset-0 z-50 h-screen rounded-none' : ''}`}>
      <div className={`${colors.headerBg} px-2 sm:px-3 py-1.5 flex items-center justify-between border-b ${colors.headerBorder}`}>
            ...
            {(['1m', '5m', '15m', '30m', '1h', '4h', '1d'] as const).map((tf) => {
              ...
              return (
                <button
                  ...
                  className={`px-1.5 sm:px-2 py-0.5 text-[10px] font-medium rounded transition-all flex-shrink-0 ${
                    timeframe === tf
