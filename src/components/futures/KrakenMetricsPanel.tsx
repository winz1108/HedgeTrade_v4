...
const getExitReasonColor = (reason?: string): { bg: string; text: string; border: string } => {
  ...
  if (reason === 'VANISH') return { bg: 'bg-orange-900/30', text: 'text-orange-400', border: 'border-orange-700' };
  if (reason === 'TIMEOUT') return { bg: 'bg-cyan-900/30', text: 'text-cyan-400', border: 'border-cyan-700' };
  return { bg: 'bg-slate-700/30', text: 'text-slate-400', border: 'border-slate-600' };
};
...
export function KrakenMetricsPanel({ data, position }: Props) {
      ...
      <div className="flex flex-col gap-1.5">
        <div className="bg-slate-800/95 border border-slate-700 rounded-lg shadow-sm p-2">
          <div className="flex items-center justify-between mb-1.5">
            <h3 className="text-[11px] font-bold text-slate-200">Status</h3>
            <Activity className="w-3 h-3 text-slate-400" />
          </div>

          <div className="space-y-1.5">
            <div className="bg-amber-900/20 rounded-lg p-2 border border-amber-700/50">
              <div className="text-[10px] text-amber-400 font-medium mb-0.5">TOTAL ASSET</div>
              <div className="text-xl font-bold text-slate-100 mb-1">
                {formatCurrency(data.balance.portfolioValue)}
              </div>
              <div className="space-y-0.5 pt-1 border-t border-amber-700/50">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-[9px] text-slate-400">Leverage</span>
            ...
            <div className="border-t border-slate-700 pt-1.5">
              <div className="text-[10px] text-slate-300 mb-1 font-medium">POSITION</div>
              {hasPosition && entryPrice ? (
                <div className="space-y-0.5 bg-cyan-900/20 rounded-lg p-1.5 border border-cyan-700/50">
                  <div className="flex justify-between items-center">
                    <span className="text-[9px] text-cyan-400">Side</span>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-[9px] text-cyan-400">Entry</span>
                    <span className="text-[11px] font-bold text-slate-200">{formatCurrency(entryPrice)}</span>
                  </div>
                  {liquidationPrice && (
                    <div className="flex justify-between items-center">
                      <span className="text-[9px] text-orange-400">Liquidation</span>
                      <span className="text-[11px] font-bold text-orange-400">{formatCurrency(liquidationPrice)}</span>
                    </div>
                  )}
                  {currentPnl !== undefined && (
                    <div className="flex justify-between items-center">
                      <span className="text-[9px] text-cyan-400">P&L</span>
                      <span className={`text-[11px] font-bold ${
                  ...
                  {data.strategyA?.entry_time && (
                    <div className="flex justify-between items-center">
                      <span className="text-[9px] text-cyan-400">Duration</span>
                ...
                <div className="px-2 py-1 bg-slate-700/50 text-slate-400 rounded text-[10px] font-bold inline-block border border-slate-600">
                  NO POSITION
                </div>
              )}
            </div>
        ...
        <div className="bg-slate-800/95 border border-emerald-700/50 rounded-lg shadow-sm p-2">
          <div className="flex items-center justify-between mb-1.5">
            <h3 className="text-[11px] font-bold text-slate-200">Entry Conditions</h3>
                ...
                return (
                  ...
                  >
                    <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${
                      met ? 'bg-emerald-400' : 'bg-slate-500'
                    }`} />
  ...
  if (position === 'right') {
    ...
    return (
      <div className="flex flex-col gap-1.5">
        <div className="bg-slate-800/95 border border-amber-700/50 rounded-lg shadow-sm p-2">
          <div className="flex items-center justify-between mb-1.5">
            <h3 className="text-[11px] font-bold text-slate-200">Performance</h3>
          ...
          <div className="space-y-1.5">
            {data.metrics?.portfolioReturnWithCommission !== undefined && (
              <div className="bg-gradient-to-br from-emerald-900/40 to-teal-900/40 p-2 rounded-lg border border-emerald-700/50">
                <div className="text-[10px] text-emerald-400 font-bold mb-0.5">NET PROFIT</div>
                <div
            ...
            <div className="space-y-0.5">
              <div className="flex justify-between items-center bg-amber-900/20 p-1 rounded border border-amber-700/50">
                <span className="text-[9px] text-slate-300 font-medium">Portfolio Return</span>
                <span className={`text-[11px] font-bold ${
              ...
              <div className="flex justify-between items-center bg-amber-900/20 p-1 rounded border border-amber-700/50">
                <span className="text-[9px] text-slate-300 font-medium">Market Change</span>
                <span className={`text-[11px] font-bold ${
              ...
              <div className="flex justify-between items-center bg-amber-900/20 p-1 rounded border border-amber-700/50">
                <span className="text-[9px] text-slate-300 font-medium">Avg Trade Return</span>
                <span className={`text-[11px] font-bold ${
        ...
        <div className="bg-slate-800/95 border border-cyan-700/50 rounded-lg shadow-sm p-2">
          <div className="flex items-center justify-between mb-1.5">
            <h3 className="text-[11px] font-bold text-slate-200">Statistics</h3>
          ...
          <div className="space-y-1">
            ...
            <div className="border-t border-cyan-700/50 pt-1 mt-1">
              <div className="text-[10px] text-slate-300 mb-1 font-bold">WIN RATE</div>
                ...
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      <span className="text-xs font-bold text-amber-400 min-w-[40px]">
                        {winRate.toFixed(1)}%
                      </span>
                    </>
                  );
  ...
  if (position === 'trades') {
    ...
    return (
      <div className="bg-slate-800/95 border border-amber-700/50 rounded-lg shadow-sm p-2 flex flex-col" style={{ height: '100%' }}>
        <div className="flex items-center justify-between mb-1">
          <h3 className="text-[11px] font-bold text-slate-200">Recent Trades</h3>
          ...
          {recentTrades.length > 0 ? (
            recentTrades.map((trade, index) => (
              <div key={`${trade.timestamp}-${index}`}>
                {trade.type === 'buy' ? (
                  <div className="bg-cyan-900/20 border border-cyan-700/50 rounded p-1">
