...
import { KrakenDashboardData, DashboardData, TradeEvent, BosLevel } from '../../types/dashboard';
...
export function KrakenPriceChart({ data, onTimeframeChange, zbZones, zbStatus, binancePrice }: Props) {
    ...
    const getCandles = (timeframe: '1m' | '5m' | '15m' | '30m' | '1h' | '4h' | '1d' | 'cvb'): any[] => {
      ...
      let processedCandles = candles.map(c => {
        ...
        return {
          ...c,
          timestamp: toMs(raw),
          swing_high: ind.swing_high ?? c.swing_high ?? false,
          swing_low: ind.swing_low ?? c.swing_low ?? false,
          ema_short: ind.ema_short ?? ind.ema20 ?? c.ema_short ?? c.ema20,
          ema_long: ind.ema_long ?? ind.ema50 ?? c.ema_long ?? c.ema50,
          ema200: ind.ema200 ?? c.ema200,
          bb_upper: ind.bb_upper ?? c.bb_upper,
          bb_mid: ind.bb_mid ?? c.bb_mid,
          bb_lower: ind.bb_lower ?? c.bb_lower,
          adx: ind.adx ?? c.adx,
          rsi: ind.rsi ?? c.rsi,
          macd: ind.macd ?? ind.macd_line ?? c.macd ?? c.macd_line,
          signal: ind.signal ?? ind.macd_signal ?? c.signal ?? c.macd_signal,
          histogram: ind.histogram ?? ind.macd_hist ?? c.histogram ?? c.macd_hist,
        };
      });
  ...
  if (bosLevels) {
    console.log('[BOS Debug] Kraken bosLevels count:', bosLevels.length, 'sample:', bosLevels[0]);
  }
