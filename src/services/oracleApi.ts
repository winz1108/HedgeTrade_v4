  ...
  if (apiResponse.accounts && apiResponse.accounts.length > 0) {
    ...
    const mapCandles = (candles: any[]) => {
      ...
      return candles.map(c => ({
        ...
        ema20: c.ema20 ?? c.ema_short ?? c.ema8,
        ema50: c.ema50 ?? c.ema_long ?? c.ema13,
        ema200: c.ema200,
        ema3: c.ema3,
        ema8: c.ema8,
        bb_upper: c.bb_upper,
...
const normalizeKrakenCandles = (candles: any[]): Candle[] => {
  ...
  return candles.map(candle => ({
    ...
    ema5: candle.ema5,
    ema13: candle.ema13,
    ema20: candle.ema20 ?? candle.ema_short ?? candle.ema8,
    ema50: candle.ema50 ?? candle.ema_long ?? candle.ema13,
...
export const fetchBinanceFuturesChartData = async (timeframe: string, limit: number = 500) => {
  ...
  try {
    ...
    const mapCandles = (candles: any[]): any[] => candles?.map(c => ({
      ...
      ema_long: c.ema_long,
      ema20: c.ema20 ?? c.ema_short,
      ema50: c.ema50 ?? c.ema_long,
      ema200: c.ema200,
      ema3: c.ema3,
      ema8: c.ema8,
      macd: c.macd,
